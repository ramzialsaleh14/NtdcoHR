...
const renderStatusList = (statusList) => {
  return statusList.map((status) => {
    if (status.includes("Denied")) return <Text style={{ color: "#FF0000" }}> {status} </Text>
    if (status.includes("Pending")) return <Text style={{ color: "#FFCD01" }}> {status} </Text>
    if (status.includes("Approved")) return <Text style={{ color: "#00FF00" }}> {status} </Text>
  })
}

export default function MyRequestsAdmin({ route }) {
  ...
  const allRequests = async () => {
    ...
    const reqs = await ServerOperations.getMyRequestsAdmin(userID, fromDate, toDate, status);
    setRequests(reqs);
  };
          ...
          <View style={styles.cardContainer}>
            <Card>
              ...
              <Card.Content>
                ...
                {!!(item.adminNotes !== "") ? (
                  <Text style={[styles.textItemsStyle, { alignSelf: "center" }]}>
                    {" "}
                    {item.adminNotes}{" "}
                  </Text>
                ) : null}
                ...
                />
                <Text style={{ alignSelf: "center", marginVertical: 5, fontWeight: "bold", color: "gray", textDecorationLine: "underline", marginBottom: 15 }}>
                  {i18n.t("response")}
                </Text>


                {!!(
                  item.respAttachment !== "" && item.respAttachment !== "undefined"
                ) ? (
                  <View>
                    <Button
                      mode="outlined"
                ...
                {!!(item.respNotes !== "") ? (
                  <Text style={styles.textItemsStyle}>
                    {" "}
                    {i18n.t("respNotes")} {":"} {item.respNotes}{" "}
                  </Text>
                ) : null}
                ...
                {!!(item.status !== "") ? (
                  <Text style={{ marginLeft: 5, padding: 5 }}>
                    ...
                    >
                      {" "}
                      {(item.status == "Approved" ? "موافق عليه" : item.status == "Denied" ? "مرفوض" : "معلق")}{" "}
                    </Text>
                  </Text>
                ) : null}
              </Card.Content>
