...
const ViewRequestsScreen = ({ route }) => {
  ...
  const allRequests = async () => {
    ...
    const userNoKey = route.params.userNoKey;
    const statusKey = route.params.statusKey;
    const reqs = await ServerOperations.getAllRequests(userID, fromDate, toDate, userNoKey, statusKey);
    setRequests(reqs);
  ...
  const respondToReq = async (resp) => {
    setProgressDialogVisible(true);
    const userID = await Commons.getFromAS("userID");
    const res = await ServerOperations.respondToRequest(
      userID,
      resp,
      curItem.id,
      respNotes
    ...
    if (res.res == "SENT") {
      ...
      Commons.okAlert("", i18n.t("checkConnectionErr"));
      setProgressDialogVisible(false);
    }
    setProgressDialogVisible(false);
  };
