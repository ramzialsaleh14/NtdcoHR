...
export default function ViewAdminHRScreen({ route }) {
  ...
  const renderStatusList = (statusList) => {
    return statusList.map((status) => {
      if (status.includes("Denied")) return <Text style={{ color: "#FF0000" }}> {status.replace("Denied", "مرفوض")} </Text>
      if (status.includes("Pending")) return <Text style={{ color: "#FFCD01" }}> {status.replace("Pending", "معلق")} </Text>
      if (status.includes("Approved")) return <Text style={{ color: "#00FF00" }}> {status.replace("Approved", "موافق عليه")} </Text>
    })
  }
  ...
  const allRequests = async () => {
    ...
    const reqs = await ServerOperations.getAdminRequestsHR(
      userID,
      fromDate,
      toDate,
      userNo,
              ...
              <Card.Content>
                ...
                {!!(item.respNotes !== "") ? (
                  <Text style={{ marginLeft: 5, padding: 5 }}>
                    <Text style={styles.textItemsStyle}> {item.respNotes} </Text>
                  </Text>
                ) : null}
